...
<div class="p-6 max-w-2xl mx-auto">
    ...
    <div class="flex items-center gap-3 mb-6">
        <a href="{{ route('fournisseurs.show', $fournisseur) }}" class="text-stone-400 hover:text-stone-600">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/>
            </svg>
        </a>
        <div>
            <h2 class="text-2xl font-semibold text-stone-800">Modifier le fournisseur</h2>
            <p class="text-sm text-stone-500">{{ $fournisseur->nom }}</p>
        </div>
    </div>
        ...
        <div class="bg-white border border-stone-200 rounded-xl p-5 space-y-4">
            ...
            <div>
                ...
                <textarea name="adresse" rows="2"
                          class="w-full border border-stone-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-amber-500">{{ old('adresse', $fournisseur->adresse) }}</textarea>
            </div>
